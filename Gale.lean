...
structure OChart (X : Type*) [TopologicalSpace X]
  extends OpenPartialHomeomorph X NNReal where
...
lemma overlap_oo_is_outer {U : OChart α} {V : OChart α} (h : Overlap U.source V.source)
  ...
  IsOuter ((U.toFun') '' (connectedComponentIn (U.source ∩ V.source) x)) := by
      ...
      have huc : IsConnected U.source := by
        apply (partial_homeo_source_connected_iff_target_connected U.toOpenPartialHomeomorph).mpr
        rw [U.target_ioo]
      ...
      have hwv : ((closure W) ∩ (V.source \ W)).Nonempty := by
        have hVconn : IsConnected V.source := by
          apply (partial_homeo_source_connected_iff_target_connected V.toOpenPartialHomeomorph).mpr
          rw [V.target_ioo]
        ...
        have hVU : (V.source \ W).Nonempty := by
          ...
          constructor
          · exact Set.mem_of_mem_sdiff hy
          · by_contra hw
            have : y ∈ U.source := hm hw
            simp only [Set.mem_sdiff] at hy
            exact hy.2 this
