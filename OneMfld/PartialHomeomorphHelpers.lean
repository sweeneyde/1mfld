...
lemma partial_homeo_connected {X : Type*} {Y : Type*} [TopologicalSpace X] [TopologicalSpace Y]
  (h : PartialHomeomorph X Y) (conn : IsConnected h.source) : (IsConnected h.target) := by
...
lemma partial_homeo_connected' {X : Type*} {Y : Type*} [TopologicalSpace X] [TopologicalSpace Y]
  (h : PartialHomeomorph X Y) (conn : IsConnected h.target) : (IsConnected h.source) := by
...
lemma partial_homeo_source_connected_iff_target_connected {X : Type*} {Y : Type*} [TopologicalSpace X] [TopologicalSpace Y]
  (h : PartialHomeomorph X Y) : IsConnected h.source ↔ IsConnected h.target := by
  constructor
